...
struct register_state
{
  ...
  uint64_t
      neon[32][2];  // q0-q31 (full 128-bit NEON registers as two 64-bit values)
};
...
extern void aarch64_call_stub(struct register_state *input,
                              struct register_state *output,
                              void (*function_ptr)(void));
#else
// Stub implementation for non-AArch64 platforms
static inline void aarch64_call_stub(struct register_state *input,
                                     struct register_state *output,
                                     void (*function_ptr)(void))
{
  (void)input;
