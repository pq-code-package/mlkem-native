 ...
 * assert the checker reports a non-zero violation count - that proves the
 ...
 * flagged (that would require parsing stderr); proving the count is non-
 * zero per known-bad input plus zero on a known-good no-op is sufficient
 * to validate the checker's polarity and basic plumbing.
 *
...
} selftest_aarch64_entry;

static const selftest_aarch64_entry aarch64_entries[] = {
...
/* Defined in selftest_x86_64.S. MLK_SYSV_ABI is repeated on the registry's
 * fn field below so table assignments stay well-typed on Windows-MinGW. */
extern MLK_SYSV_ABI
...
} selftest_x86_64_entry;

static const selftest_x86_64_entry x86_64_entries[] = {
    {"noop", selftest_x86_64_noop, 0},
    {"corrupt_rbx", selftest_x86_64_corrupt_rbx, 1},
    {"corrupt_rbp", selftest_x86_64_corrupt_rbp, 1},
    {"corrupt_r12", selftest_x86_64_corrupt_r12, 1},
    {"corrupt_r13", selftest_x86_64_corrupt_r13, 1},
    {"corrupt_r14", selftest_x86_64_corrupt_r14, 1},
    {"corrupt_r15", selftest_x86_64_corrupt_r15, 1},
    {NULL, NULL, 0},
};
...
/* CR2 corrupter exercises the MLK_PPC_CR_NV_MASK path; CR3/CR4 share it. */
...
typedef struct
{
  ...
  void (*fn)(void);
  int expected_violations;
} selftest_ppc64le_entry;

static const selftest_ppc64le_entry ppc64le_entries[] = {
...
typedef struct
{
  ...
  void (*fn)(void);
  int expected_violations;
} selftest_armv81m_entry;

static const selftest_armv81m_entry armv81m_entries[] = {
...
int abicheck_selftest(void)
{
...
#if defined(MLK_SYS_AARCH64)
  {
    ...
    const selftest_aarch64_entry *e;
    for (e = aarch64_entries; e->name != NULL; e++)
    {
      int violations;
      init_register_state(&input_state);
      asm_call_stub_aarch64(&input_state, &output_state, e->fn);
      violations = check_aarch64_aapcs_compliance(&input_state, &output_state,
                                                  MLK_ABICHECK_QUIET);
      if ((violations == 0) != (e->expected_violations == 0))
      {
        fprintf(stderr,
                "selftest FAIL: aarch64 %s: expected %s, got %d violations\n",
                e->name, e->expected_violations ? "violations" : "clean",
                violations);
        failures++;
      }
    }
...
#elif defined(MLK_SYS_X86_64) && defined(MLK_SYSV_ABI_SUPPORTED)
  {
    struct x86_64_register_state input_state, output_state;
    const selftest_x86_64_entry *e;
    for (e = x86_64_entries; e->name != NULL; e++)
    {
      int violations;
      init_x86_64_register_state(&input_state);
      asm_call_stub_x86_64_sysv(&input_state, &output_state, e->fn);
      violations = check_x86_64_sysv_compliance(&input_state, &output_state,
                                                MLK_ABICHECK_QUIET);
      if ((violations == 0) != (e->expected_violations == 0))
      {
        fprintf(stderr,
                "selftest FAIL: x86_64 %s: expected %s, got %d violations\n",
                e->name, e->expected_violations ? "violations" : "clean",
                violations);
        failures++;
      }
...
#elif defined(MLK_SYS_PPC64LE)
  {
    struct ppc64le_register_state input_state, output_state;
    const selftest_ppc64le_entry *e;
    for (e = ppc64le_entries; e->name != NULL; e++)
    {
      int violations;
      init_ppc64le_register_state(&input_state);
      asm_call_stub_ppc64le(&input_state, &output_state, e->fn);
      violations = check_ppc64le_elfv2_compliance(&input_state, &output_state,
                                                  MLK_ABICHECK_QUIET);
      if ((violations == 0) != (e->expected_violations == 0))
      {
        fprintf(stderr,
                "selftest FAIL: ppc64le %s: expected %s, got %d violations\n",
                e->name, e->expected_violations ? "violations" : "clean",
                violations);
        failures++;
      }
...
#elif defined(MLK_SYS_ARMV81M_MVE)
  {
    struct armv81m_register_state input_state, output_state;
    const selftest_armv81m_entry *e;
    for (e = armv81m_entries; e->name != NULL; e++)
    {
      int violations;
      init_armv81m_register_state(&input_state);
      asm_call_stub_armv81m(&input_state, &output_state, e->fn);
      violations = check_armv81m_aapcs32_compliance(&input_state, &output_state,
                                                    MLK_ABICHECK_QUIET);
      if ((violations == 0) != (e->expected_violations == 0))
      {
        fprintf(stderr,
                "selftest FAIL: armv81m %s: expected %s, got %d violations\n",
                e->name, e->expected_violations ? "violations" : "clean",
                violations);
        failures++;
      }
