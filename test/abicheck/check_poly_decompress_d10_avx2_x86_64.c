...
void mlk_poly_decompress_d10_avx2(int16_t *r, const uint8_t *a,
                                  const uint8_t *data);
...
int check_poly_decompress_d10_avx2_x86_64(void)
{
  ...
  for (test_iter = 0; test_iter < NUM_TESTS; test_iter++)
  {
    ...
    asm_call_stub_x86_64(&input_state, &output_state,
                         (void (*)(void))mlk_poly_decompress_d10_avx2);
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for poly_decompress_d10_avx2 (iteration %d): %d "
              "violations\n",
...
MLK_EMPTY_CU(check_poly_decompress_d10_avx2_x86_64)
