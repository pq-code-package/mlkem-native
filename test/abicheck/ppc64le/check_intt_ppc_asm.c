...
void mlk_intt_ppc_asm(int16_t *r, const int16_t *qdata);
...
int check_intt_ppc_asm(void)
{
  ...
  for (test_iter = 0; test_iter < MLK_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    if (violations > 0)
    {
      fprintf(
          stderr,
          "ABI test FAILED for intt_ppc_asm (iteration %d): %d violations\n",
...
int check_intt_ppc_asm(void)
{
  fprintf(stderr, "ABI check intt_ppc_asm: unsupported architecture\n");
  return MLK_ABICHECK_FAILED;
}
