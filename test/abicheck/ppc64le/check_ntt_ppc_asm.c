...
void mlk_ntt_ppc_asm(int16_t *r, const int16_t *qdata);
...
int check_ntt_ppc_asm(void)
{
  ...
  for (test_iter = 0; test_iter < MLK_ABICHECK_NUM_TESTS; test_iter++)
  {
    ...
    {
      fprintf(stderr,
              "ABI test FAILED for ntt_ppc_asm (iteration %d): %d violations\n",
...
int check_ntt_ppc_asm(void)
{
  fprintf(stderr, "ABI check ntt_ppc_asm: unsupported architecture\n");
  return MLK_ABICHECK_FAILED;
}
