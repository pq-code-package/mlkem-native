...
starts it directly from RAM, polls ``SYSCFG->CM55TCMCR`` until the requested TCM
split is visible over SWD, then resets the target so the next ELF can be loaded
into the expanded ITCM/DTCM layout.
"""
...
def cubeprogrammer_cli():
    """Return the CubeProgrammer CLI path, reporting a helpful error if absent."""
...
def resolve_symbol(elf, symbol):
    ...
    return resolve_symbol_with_nm(elf, symbol, nm=os.environ.get("NM", "arm-none-eabi-nm"))
...
def main():
    """Download and run the FLEXMEM config ELF, then verify the latched layout."""
    configure_logging()
