...
class NucleoHostTest(unittest.TestCase):
    ...
    def test_pack_cmdline_uses_absolute_string_pointers(self):
        """The argv table contains absolute target pointers to inline strings."""
    ...
    def test_parse_readelf_symbol(self):
        """The readelf parser ignores substring matches and returns exact hits."""
        readelf_output = "   42: 00001234     8 FUNC    GLOBAL DEFAULT    1 __wrap_main\n"

        self.assertEqual(parse_readelf_symbol(readelf_output, "__wrap_main"), "0x00001234")
    ...
    def test_gdb_load_failed_detects_representative_output(self):
        ...
        self.assertTrue(gdb_load_failed("Error finishing flash operation\nLoad failed\n"))
    ...
    def test_main_recovers_once_after_load_failure(self):
        ...
        with mock.patch.dict(os.environ, env), \
             ...
             mock.patch.object(exec_wrapper, "_recover_after_load_failure", return_value=True) as recover, \
    ...
    def test_main_reports_diagnostics_when_load_recovery_fails(self):
        ...
        with mock.patch.dict(os.environ, env), \
             mock.patch.object(exec_wrapper, "_run_once", side_effect=fake_run_once), \
             mock.patch.object(exec_wrapper, "_recover_after_load_failure", return_value=False), \
             mock.patch.object(exec_wrapper, "err", side_effect=messages.append), \
             mock.patch.object(exec_wrapper.time, "sleep"):
    ...
    def test_main_does_not_recover_when_load_recovery_disabled(self):
        ...
        with mock.patch.dict(os.environ, env), \
             ...
             mock.patch.object(exec_wrapper, "err", side_effect=messages.append), \
             mock.patch.object(exec_wrapper, "log_output", side_effect=lambda msg, *args, **kwargs: messages.append(msg)):
            self.assertEqual(exec_wrapper.main(), 23)
    ...
    def test_restore_argv_command_prefers_numeric_address(self):
        """Numeric argv restore addresses are preferred over symbol expressions."""
        self.assertEqual(restore_argv_command("argv.bin", "0x1234", "mlk_cmdline_block"), "restore argv.bin binary 0x1234")
        self.assertEqual(restore_argv_command("argv.bin", None, "mlk_cmdline_block"), "restore argv.bin binary &mlk_cmdline_block")

    def test_build_run_script_contains_required_sequence(self):
        """The generated GDB script keeps the critical load/start/restore order."""
        ...
        self.assertIn("  dump binary memory stdout.bin 0x34080000 0x34080000 + $nucleo_stdout_len", gdb_lines)
