...
target output starts, the wrapper can re-run FLEXMEM configuration and retry the
same ELF. Successful runs load the ELF into RAM, let C startup clear memory,
restore a packed argv blob at ``__wrap_main``, stream or dump target stdout, and
map target sentinels to the process exit status expected by the baremetal test
harness.
"""
...
def popen(cmd, **kwargs):
    """Thin wrapper around ``subprocess.Popen`` for test-time monkeypatching."""
...
def _recover_flexmem(reason: str, failure_message: str) -> bool:
    """Re-run FLEXMEM configuration after a retryable setup or target failure."""
    platform_dir = os.path.dirname(os.path.abspath(__file__))
...
def _recover_after_hardfault() -> bool:
    ...
    return _recover_flexmem("HardFault", "FLEXMEM reconfiguration after HardFault failed")
...
def _run_once():
    """Run the target ELF once and return the wrapper exit code for that run."""
    global VERBOSE
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            def _semihost_reader(sock: socket.socket):
                """Stream semihost console output and detect the exit sentinel."""
                global STDOUT_BYTES_EMITTED
            ...
            if os.path.exists(stdout_capture_bin):
                try:
                    ...
                    if captured_output and not shared.get("stdout_streamed") and not target_failed:
            ...
            if gdbp.returncode != 0:
                target_output_observed = bool(shared.get("stdout_streamed")) or STDOUT_BYTES_EMITTED != 0
...
def main():
    ...
    load_failure_attempts = max(0, int(os.environ.get("GDB_LOAD_FAILURE_RECOVERY_ATTEMPTS", "1")))
    transport_retries = 0
        ...
        if TARGET_FAILURE_KIND == "load-failed":
            if can_retry_load_failure:
                ...
                if _recover_after_load_failure():
                    if VERBOSE:
                        err(
                            "[exec_wrapper] retrying after recovered GDB load failure "
                            f"({load_failure_recoveries}/{load_failure_attempts})"
                        )
                    time.sleep(0.5)
                    continue
