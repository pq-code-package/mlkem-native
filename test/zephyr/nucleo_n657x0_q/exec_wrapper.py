...
GDB-loads the Zephyr ELF into RAM, restores a packed argv blob after Zephyr
startup reaches ``__wrap_main``, decodes target stdout from ITM port 0, and
reads the target return code from ``r0`` at the ``nucleo_test_done``
breakpoint.
"""
...
def _run_once():
    ...
    arg_block_sym = "mlkem_cmdline_block"
    arg_block_addr = None
