 ...
 * Target stdout/stderr goes through ITM stimulus port 0 and is captured by the
 * host over SWO. The test return code is passed to nucleo_test_done(), where
 * the host wrapper reads r0.
 */
...
static int nucleo_vprintf(const char *format, va_list ap)
{
  ...
  nucleo_stdout_write(buf,
                      (size_t)((rc < (int)sizeof(buf)) ? rc : (int)sizeof(buf) - 1));
  return rc;
}
...
void __wrap_exit(int status) { nucleo_test_done(status); }
