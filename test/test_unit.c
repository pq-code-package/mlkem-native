 ...
 * functionality with different degrees of batching. This test
 * Add unit tests making sure these functions indeed produce
 * the same outputs. */
...
static int test_poly_rej_uniform_consistency(void)
{
  ...
  /* Temporary set this iteration to 5, check the performance issue. */
  for (i = 0; i < 5; i++)
  {
    for (j = 0; j < 4; j++)
    {
