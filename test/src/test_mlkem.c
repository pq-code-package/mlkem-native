...
static int test_incremental_enc(void)
{
  ...
  /* Incremental encapsulation via KEM-level API (coins-only) */
  CHECK(mlk_kem_enc_derand_u(ct_u, ss, ek_seed, hpk, enc_coins, 0) == 0);
  CHECK(mlk_kem_enc_v(ct_v, ek_vector, hpk, enc_coins, 0) == 0);
