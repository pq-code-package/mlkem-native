                          ...
                          MLK_CONFIG_CONTEXT_PARAMETER_TYPE context)
{
  ...
  ret = mlk_indcpa_enc_u(ct_u, seed, kr + MLKEM_SYMBYTES, context);
  if (ret != 0)
  {
    goto cleanup;
  }
                   ...
                   MLK_CONFIG_CONTEXT_PARAMETER_TYPE context)
{
  ...
  if (buf == NULL || kr == NULL || p == NULL || p_reencoded == NULL)
  {
    ret = MLK_ERR_OUT_OF_MEMORY;
    goto cleanup;
  }
  ...
  /* Re-derive (K, r) = G(coins || H(pk)) to obtain CPA randomness r */
  mlk_memcpy(buf, coins, MLKEM_SYMBYTES);
  mlk_memcpy(buf + MLKEM_SYMBYTES, hpk, MLKEM_SYMBYTES);
  mlk_hash_g(kr, buf, 2 * MLKEM_SYMBYTES);

  /* coins serves as the CPA message m, kr+MLKEM_SYMBYTES is r */
  ret =
      mlk_indcpa_enc_v(ct_v, coins, ek_vector, kr + MLKEM_SYMBYTES, context);
