  ...
  ensures(array_abs_bound(r, 0, MLKEM_N, NTT_BOUND7)))
{
  ...
  {
    const int16_t zeta = layer6_zetas[j];
  ...
  ensures(array_abs_bound(r, 0, MLKEM_N, NTT_BOUND8)))
{
  ...
  {
    const int32_t zeta = (int32_t)layer7_zetas[j];
...
)
{
  ...
  {
    /* Process 4 coefficients here, all of which need the same Zeta value */
    const int32_t zeta = (int32_t)layer7_zetas[63 - i];
...
)
{
  ...
  {
    const int32_t zeta = (int32_t)layer6_zetas[31 - i];
