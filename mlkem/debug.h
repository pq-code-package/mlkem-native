...
#define mlk_debug_assert_bound(ptr, len, value_lb, value_ub)                \
...
#define mlk_debug_assert_abs_bound(ptr, len, value_abs_bd) \
  mlk_debug_assert_bound((ptr), (len), (-(value_abs_bd) + 1), (value_abs_bd))

/* Version of bounds assertions for 2-dimensional arrays */
#define mlk_debug_assert_bound_2d(ptr, len0, len1, value_lb, value_ub) \
  mlk_debug_assert_bound((ptr), ((len0) * (len1)), (value_lb), (value_ub))

#define mlk_debug_assert_abs_bound_2d(ptr, len0, len1, value_abs_bd) \
  mlk_debug_assert_abs_bound((ptr), ((len0) * (len1)), (value_abs_bd))

/* When running CBMC, convert debug assertions into proof obligations */
#elif defined(CBMC)
#include "cbmc.h"

#define mlk_debug_assert(val) cassert(val)
...
#define mlk_debug_assert_bound_2d(ptr, M, N, value_lb, value_ub)       \
  cassert(forall(kN, 0, (M),                                           \
                 array_bound(&((int16_t(*)[(N)])(ptr))[kN][0], 0, (N), \
                             (value_lb), (value_ub))))

#define mlk_debug_assert_abs_bound_2d(ptr, M, N, value_abs_bd)             \
  cassert(forall(kN, 0, (M),                                               \
