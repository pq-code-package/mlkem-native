...
#define mlk_mlk_keccak_absorb_once MLK_NAMESPACE(keccak_absorb_once)
#define mlk_mlk_keccak_squeeze_once MLK_NAMESPACE(keccak_squeeze_once)
#define mlk_mlk_keccak_squeezeblocks MLK_NAMESPACE(keccak_squeezeblocks)
/* End of static namespacing */

/*************************************************
 * Name:        mlk_mlk_keccak_absorb_once
 *
 ...
 **************************************************/
static void mlk_mlk_keccak_absorb_once(uint64_t *s, uint32_t r,
...
/*************************************************
 * Name:        mlk_mlk_keccak_squeezeblocks
 *
 ...
 *              - uint32_t r: rate in bytes (e.g., 168 for SHAKE128)
 **************************************************/
static void mlk_mlk_keccak_squeezeblocks(uint8_t *h, size_t nblocks,
...
/*************************************************
 * Name:        mlk_mlk_keccak_squeeze_once
 *
 ...
 *              - uint32_t r: rate in bytes (e.g., 168 for SHAKE128)
 **************************************************/
static void mlk_mlk_keccak_squeeze_once(uint8_t *h, size_t outlen, uint64_t *s,
                                        uint32_t r)
__contract__(
    requires(r <= sizeof(uint64_t) * KECCAK_LANES)
    requires(memory_no_alias(s, sizeof(uint64_t) * KECCAK_LANES))
...
void mlk_shake128_absorb_once(mlk_shake128ctx *state, const uint8_t *input,
                              size_t inlen)
{
  mlk_mlk_keccak_absorb_once(state->ctx, SHAKE128_RATE, input, inlen, 0x1F);
}

void mlk_shake128_squeezeblocks(uint8_t *output, size_t nblocks,
                                mlk_shake128ctx *state)
{
  mlk_mlk_keccak_squeezeblocks(output, nblocks, state->ctx, SHAKE128_RATE);
}
                  ...
                  size_t inlen)
{
  ...
  /* Absorb input */
  mlk_mlk_keccak_absorb_once(state.ctx, SHAKE256_RATE, input, inlen, 0x1F);
  /* Squeeze output */
  mlk_mlk_keccak_squeeze_once(output, outlen, state.ctx, SHAKE256_RATE);
  /* Specification: Partially implements
   * [FIPS 203, Section 3.3, Destruction of intermediate values] */
...
void mlk_sha3_256(uint8_t *output, const uint8_t *input, size_t inlen)
{
  uint64_t ctx[25];
  /* Absorb input */
  mlk_mlk_keccak_absorb_once(ctx, SHA3_256_RATE, input, inlen, 0x06);
  /* Squeeze output */
  mlk_mlk_keccak_squeeze_once(output, 32, ctx, SHA3_256_RATE);
  /* Specification: Partially implements
   * [FIPS 203, Section 3.3, Destruction of intermediate values] */
...
void mlk_sha3_512(uint8_t *output, const uint8_t *input, size_t inlen)
{
  uint64_t ctx[25];
  /* Absorb input */
  mlk_mlk_keccak_absorb_once(ctx, SHA3_512_RATE, input, inlen, 0x06);
  /* Squeeze output */
  mlk_mlk_keccak_squeeze_once(output, 64, ctx, SHA3_512_RATE);
  /* Specification: Partially implements
   * [FIPS 203, Section 3.3, Destruction of intermediate values] */
  mlk_ct_zeroize(ctx, sizeof(ctx));
}
...
#undef mlk_mlk_keccak_absorb_once
#undef mlk_mlk_keccak_squeeze_once
#undef mlk_mlk_keccak_squeezeblocks
