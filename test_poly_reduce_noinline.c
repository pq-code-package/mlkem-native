...
void process_vector_chunk_noinline(int16_t *r, size_t i,
                                   size_t vl);  // NON-STATIC NON-INLINE
static void poly_reduce_noinline(int16_t *r);
static int test_poly_reduce_noinline_bounds(int16_t *test_values,
                                            int num_values,
...
// NON-STATIC NON-INLINE function that processes one vector chunk
// This hoists the exact sequence from the original code:
//   vt = __riscv_vle16_v_i16m1(&r[i], vl);
//   vt = fq_barrett(vt, vl);
//   vt = fq_cadd(vt, vl);
//   __riscv_vse16_v_i16m1(&r[i], vt, vl);
void process_vector_chunk_noinline(int16_t *r, size_t i, size_t vl)
{
  vint16m1_t vt;

  printf("[DEBUG] Processing chunk at index %zu (non-inline function)\n", i);
  ...
  // The exact sequence from the original code - now in a non-inlined function
  vt = __riscv_vle16_v_i16m1(&r[i], vl);
  vt = fq_barrett(vt, vl);
  vt = fq_cadd(vt, vl);
  __riscv_vse16_v_i16m1(&r[i], vt, vl);
...
// Standalone poly_reduce function using non-inline vector processing
static void poly_reduce_noinline(int16_t *r)
{
  ...
  printf(
      "[DEBUG] Processing %d coefficients in chunks of %zu (using NON-INLINE "
      "function)\n",
      MLKEM_N, vl);

  for (size_t i = 0; i < MLKEM_N; i += vl)
  {
    // Call the NON-STATIC NON-INLINE function that does the actual work
    process_vector_chunk_noinline(r, i, vl);
  }
}

// Test function that checks bounds after poly_reduce with non-inline processing
static int test_poly_reduce_noinline_bounds(int16_t *test_values,
                                            int num_values,
                                            const char *test_name)
{
  printf("\n========================================\n");
  printf("Testing %s (%d values) with NON-INLINE vector processing:\n",
  ...
  // Apply poly_reduce with non-inline vector processing
  poly_reduce_noinline(poly);
...
{
  printf(
      "NON-INLINE Vector poly_reduce test (checking for compiler inlining "
      "bugs)\n");
  ...
  if (test_poly_reduce_noinline_bounds(problematic, 14,
                                       "Problematic values (NON-INLINE)"))
  {
    passed_tests++;
  }
  total_tests++;
  ...
  if (test_poly_reduce_noinline_bounds(full_poly, MLKEM_N,
                                       "Full polynomial (NON-INLINE)"))
  {
    passed_tests++;
  }
  total_tests++;
  ...
  {
    printf(
        "✓ All tests PASSED! NON-INLINE vector poly_reduce works correctly.\n");
    printf("  This suggests NO compiler inlining bug.\n");
    return 0;
  }
  else
  {
    printf(
        "✗ %d test groups FAILED! NON-INLINE vector poly_reduce has issues.\n",
        total_tests - passed_tests);
    printf("  This might indicate a compiler inlining bug!\n");
    return 1;
  }
}
