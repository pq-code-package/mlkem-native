...
mlkem-native v1.3.0 adds fine-grained control over which KEM operations are
included in a build, improves AArch64 feature detection and runtime dispatch,
and introduces ABI compliance checks for assembly backends. Embedded testing
now uses Zephyr across a broader range of Arm targets, while assembly
...
- Strengthen HOL Light tooling with cross-architecture proof execution and
  checks that every expected theorem is present, and upgrade CBMC to v6.10.0.
...
- Extend Windows MSVC coverage to allocation-failure, RNG-failure, and
  Wycheproof tests, and add emulated testing on x86_64 without AVX2.
  ([#1738](https://github.com/pq-code-package/mlkem-native/pull/1738),
...
- Update the tested AWS-LC integration from v5.0.0 to v5.4.0.
  ([#1831](https://github.com/pq-code-package/mlkem-native/pull/1831))
