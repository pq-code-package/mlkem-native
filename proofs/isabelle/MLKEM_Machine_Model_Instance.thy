...
lemma c_short_byte_prism_embed_length:
  ...
  unfolding c_short_byte_prism_def prism_compose_def word_sword_iso_prism_def iso_prism_def
    word16_byte_list_prism_le_def list_fixlen_prism_def
  by (simp add: list_fixlen_embed_def)
...
next
  ...
  obtain a b where ab: \<open>prism_embed c_short_byte_prism c = [a, b]\<close>
    using c_short_byte_prism_embed_length[of c]
    by (metis (no_types, opaque_lifting) length_0_conv length_Cons length_tl list.sel(3)
        numeral_2_eq_2 Suc_length_conv)
...
next
  case (3 a b rest)
  from 3(2) obtain c where
    proj: \<open>prism_project c_short_byte_prism [a, b] = Some c\<close> and
    mc: \<open>map_option ((#) c) (decode_c_short_list rest) = Some cs\<close>
    by (cases \<open>prism_project c_short_byte_prism [a, b]\<close>) auto
  from mc obtain cs' where
    rest_decode: \<open>decode_c_short_list rest = Some cs'\<close> and
...
lemma c_short_list_byte_prism_valid:
  ...
  unfolding is_valid_prism_def c_short_list_byte_prism_def
  by (auto simp: decode_encode_c_short_list dest: encode_decode_c_short_list)
...
lemma c_mlk_poly_struct_prism_valid:
  ...
  unfolding c_mlk_poly_struct_prism_def
  by (rule iso_prism_valid) auto

lemma c_mlk_poly_mulcache_struct_prism_valid:
  shows \<open>is_valid_prism c_mlk_poly_mulcache_struct_prism\<close>
  unfolding c_mlk_poly_mulcache_struct_prism_def
  by (rule iso_prism_valid) auto
...
definition c_mlk_poly_mulcache_byte_prism :: \<open>(8 word list, c_mlk_poly_mulcache) prism\<close> where
  \<open>c_mlk_poly_mulcache_byte_prism \<equiv> prism_compose c_short_list_byte_prism c_mlk_poly_mulcache_struct_prism\<close>

lemma c_mlk_poly_byte_prism_valid:
  shows \<open>is_valid_prism c_mlk_poly_byte_prism\<close>
  unfolding c_mlk_poly_byte_prism_def
  by (intro prism_compose_valid c_short_list_byte_prism_valid c_mlk_poly_struct_prism_valid)

lemma c_mlk_poly_mulcache_byte_prism_valid:
  shows \<open>is_valid_prism c_mlk_poly_mulcache_byte_prism\<close>
  unfolding c_mlk_poly_mulcache_byte_prism_def
  by (intro prism_compose_valid c_short_list_byte_prism_valid c_mlk_poly_mulcache_struct_prism_valid)
...
text \<open>
  ...
  parameters from the \<^verbatim>\<open>for\<close> clause.
\<close>
...
proof -
  show \<open>c_mlk_machine_model
    ...
    apply standard
    \<comment> \<open>7 prisms \<times> (is_valid_prism + can_create_gref_for_prism) = 14 goals\<close>
    apply (simp_all add:
      c_mlk_poly_byte_prism_valid c_mlk_poly_mulcache_byte_prism_valid
      c_byte_prism_valid c_short_list_byte_prism_valid
      References.can_create_gref_for_prism_def)
    done
