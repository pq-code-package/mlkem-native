...
def mk_micro_c_file_command(unit: dict, c_file: str, manifest_output: str | None) -> str:
    """Build a single micro_c_file command string."""
    parts: list[str] = []
    prefix = unit.get("prefix")
    if prefix:
        parts.append(f"prefix: {prefix}")
    ...
    if opts:
        opts += " "
    return f"micro_c_file {opts}{quote_isabelle_string(c_file)}\n"
...
def main() -> int:
    ...
    for unit in selected:
        ...
        if args.action in {"theory", "all"}:
            ...
            micro_c_file_commands = mk_micro_c_file_command(
                unit, unit["output_c"], manifest_output
            )
            rendered = render_template(
                template_path,
                {
                    ...
                    "MICRO_C_FILE_COMMANDS": micro_c_file_commands,
