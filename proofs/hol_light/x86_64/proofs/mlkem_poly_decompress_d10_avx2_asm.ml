...
let mlkem_poly_decompress_d10_mc =
  define_assert_from_elf "mlkem_poly_decompress_d10_mc" "x86_64/mlkem/mlkem_poly_decompress_d10_avx2_asm.o"
(*** BYTECODE START ***)
[
...
let MLKEM_POLY_DECOMPRESS_D10_CORRECT = prove(
  ...
  (*** Symbolic execution ***)
  MAP_UNTIL_TARGET_PC (fun n -> X86_STEPS_TAC MLKEM_POLY_DECOMPRESS_D10_TMC_EXEC [n] THEN SIMD_SIMPLIFY_TAC (map GSYM (BASE_SIMPS_D10 @ WORD_MUL_2EXP @ WORD_MUL_2EXP_3329))
                      THEN SIMP_DECOMPRESS_D10_TAC) 1 THEN
