...
# Stack analysis targets - build with stack usage flags and stack test binaries
stack_512: CFLAGS += -fstack-usage
stack_512: $(MLKEM512_DIR)/bin/stack_test512

stack_768: CFLAGS += -fstack-usage
stack_768: $(MLKEM768_DIR)/bin/stack_test768

stack_1024: CFLAGS += -fstack-usage
stack_1024: $(MLKEM1024_DIR)/bin/stack_test1024

stack: stack_512 stack_768 stack_1024

# Stack test binaries - use notrandombytes for deterministic randomness
$(MLKEM512_DIR)/bin/stack_test512: test/stack_test.c $(BUILD_DIR)/libmlkem512.a $(call MAKE_OBJS, $(MLKEM512_DIR), $(wildcard test/notrandombytes/*.c))
	$(Q)mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -DMLKEM_K=2 -Imlkem/src -o $@ $< $(BUILD_DIR)/libmlkem512.a $(call MAKE_OBJS, $(MLKEM512_DIR), $(wildcard test/notrandombytes/*.c))

$(MLKEM768_DIR)/bin/stack_test768: test/stack_test.c $(BUILD_DIR)/libmlkem768.a $(call MAKE_OBJS, $(MLKEM768_DIR), $(wildcard test/notrandombytes/*.c))
	$(Q)mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -DMLKEM_K=3 -Imlkem/src -o $@ $< $(BUILD_DIR)/libmlkem768.a $(call MAKE_OBJS, $(MLKEM768_DIR), $(wildcard test/notrandombytes/*.c))

$(MLKEM1024_DIR)/bin/stack_test1024: test/stack_test.c $(BUILD_DIR)/libmlkem1024.a $(call MAKE_OBJS, $(MLKEM1024_DIR), $(wildcard test/notrandombytes/*.c))
	$(Q)mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -DMLKEM_K=4 -Imlkem/src -o $@ $< $(BUILD_DIR)/libmlkem1024.a $(call MAKE_OBJS, $(MLKEM1024_DIR), $(wildcard test/notrandombytes/*.c))

run_stack_512: stack_512
	$(Q)python3 scripts/analyze_stack.py $(MLKEM512_DIR)/bin/stack_test512 --build-dir $(MLKEM512_DIR) $(STACK_ANALYSIS_FLAGS)
run_stack_768: stack_768
	$(Q)python3 scripts/analyze_stack.py $(MLKEM768_DIR)/bin/stack_test768 --build-dir $(MLKEM768_DIR) $(STACK_ANALYSIS_FLAGS)
run_stack_1024: stack_1024
	$(Q)python3 scripts/analyze_stack.py $(MLKEM1024_DIR)/bin/stack_test1024 --build-dir $(MLKEM1024_DIR) $(STACK_ANALYSIS_FLAGS)
